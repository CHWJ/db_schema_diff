...
function getSchema(){
	...
	echo "get create table sql..."

	# 获取建表sql
	local var1="$homeDir""get_createTable_sql.sh"
	local cmd1="mysql -h$dbIp -P$dbPort -u$dbAccount -p$dbPassword -e \"use $dbName; SHOW TABLES; \" | grep -v \"Tables_in_$dbName\" | awk -F \"\\r\\n\" '{system(\"$var1\"\" \"\$1)}'"
	eval $cmd1
